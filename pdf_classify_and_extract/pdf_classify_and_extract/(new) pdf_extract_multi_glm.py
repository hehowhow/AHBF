...
craft_prompt_dict = {
    ...
    B. 仔细阅读论文，将论文中提到的工艺步骤凝练成简洁标准的工艺流程字段，并将其归入schema中具体的step。请严格遵循以下规则：
      0. 材料提取规则：
        -识别论文中所有被沉积材料，如果论文中涉及多种材料的CVD流程，为每种材料创建独立schema，并且每个独立schema必须包含完整CVD流程
        -论文中相同工艺参数在不同材料schema中必须保持一致
        -将"_process"添加为当前schema的最外层级，命名方式为当前流程的材料名称+"_process"
      1. 严格遵循结构：
    ...
    B. 仔细阅读论文，将论文中提到的工艺步骤凝练成简洁标准的工艺流程字段，并将其归入schema中具体的step。请严格遵循以下规则：
      0. 材料提取规则：
        -识别论文中所有被沉积材料，如果论文中涉及多种材料的MOCVD流程，为每种材料创建独立schema，并且每个独立schema必须包含完整MOCVD流程
        -论文中相同工艺参数在不同材料schema中必须保持一致
        -将"_process"添加为当前schema的最外层级，命名方式为当前流程的材料名称+"_process"
      1. 严格遵循结构：
        -每个step必须保留"step_name"字段
    ...
    B. 仔细阅读论文，将论文中提到的工艺步骤凝练成简洁标准的工艺流程字段，并将其归入schema中具体的step。请严格遵循以下规则：
      0. 材料提取规则：
        -识别论文中所有被沉积材料，如果论文中涉及多种材料的MBE流程，为每种材料创建独立schema，并且每个独立schema必须包含完整MBE流程
        -论文中相同工艺参数在不同材料schema中必须保持一致
        -将"_process"添加为当前schema的最外层级，命名方式为当前流程的材料名称+"_process"
      1. 严格遵循结构：
        -每个step必须保留"step_name"字段
    ...
    B. 仔细阅读论文，将论文中提到的工艺步骤凝练成简洁标准的工艺流程字段，并将其归入schema中具体的step。请严格遵循以下规则：
      0. 材料提取规则：
        -识别论文中所有被沉积材料，如果论文中涉及多种材料的ALD流程，为每种材料创建独立schema，并且每个独立schema必须包含完整ALD流程
        -论文中相同工艺参数在不同材料schema中必须保持一致
        -将"_process"添加为当前schema的最外层级，命名方式为当前流程的材料名称+"_process"
      1. 严格遵循结构：
        -每个step必须保留"step_name"字段
...
def sort_step_fields(step_data):
    """
    确保步骤数据中的"step_name"字段位于第一个位置
    
    参数:
        step_data: 单个步骤的数据字典
    
    返回:
        排序后的步骤数据字典，"step_name"在第一位
    """
    if not isinstance(step_data, dict) or "step_name" not in step_data:
        return step_data
    
    # 先放置"step_name"，再按字母顺序放置其他字段
    ordered_items = [("step_name", step_data["step_name"])]
    remaining_items = sorted([(k, v) for k, v in step_data.items() if k != "step_name"])
    
    return dict(ordered_items + remaining_items)

def sort_process_steps(process_steps):

    # 定义排序键提取函数
    def get_step_key(step_name):
        step_name = step_name.upper()
        match = re.match(r'STEP(\d+)(?:_(\d+))?', step_name)
        if not match:
            return (float('inf'), float('inf'))
        
        main_step = int(match.group(1))
        sub_step = int(match.group(2)) if match.group(2) else 0
        return (main_step, sub_step)
    
    # 对步骤进行排序并确保step_name在第一位
    sorted_steps = []
    for step_name, step_data in sorted(process_steps.items(), key=lambda x: get_step_key(x[0])):
        sorted_step_data = sort_step_fields(step_data)
        sorted_steps.append((step_name, sorted_step_data))
    return dict(sorted_steps)

def sort_schema_keys(d, desired_order=["material", "method", "process_steps"]):
    """
      d: 要排序键的字典
      desired_order: 键的期望顺序列表
    """
    # 首先处理在期望顺序中的键
    ordered_items = [(key, d[key]) for key in desired_order if key in d]
    # 处理不在期望顺序中的键，按字母顺序排列
    remaining_items = sorted([(key, d[key]) for key in d if key not in desired_order])
    # 合并并返回新字典
    return dict(ordered_items + remaining_items)
...
def process_json_data(json_data):
    ...
    if isinstance(json_data, dict):
        if "process_steps" in json_data:
            json_data["process_steps"] = sort_process_steps(json_data["process_steps"])
        has_required_keys = all(key in json_data for key in ["material", "method", "process_steps"])
        if has_required_keys:
            json_data = sort_schema_keys(json_data)

        for key, value in json_data.items():
            json_data[key] = process_json_data(value)
    
    elif isinstance(json_data, list):
        for i in range(len(json_data)):
            json_data[i] = process_json_data(json_data[i])
    return json_data
...
# 提取工艺并保存为JSON文件
def extract_single_pdf(pdf_path, folder_process_path, api_key,log_file_path):
    """单个PDF文件抽取"""
    ...
    try:
        ...
        if not extracted_text:
            ...
            logger.warning(f"未提取到文本内容: {os.path.abspath(pdf_path)}")
            return False
        ...
        if craft_rlt is None:
            logger.error(f"工艺提取失败: {os.path.abspath(pdf_path)}")
            return False
        else:
            ...
            craft_data = f'''{craft_rlt}'''
            craft_json = json.loads(craft_data)
            craft_json_sort = process_json_data(craft_json)
            craft_json_end = json.dumps(craft_json_sort, indent=2, ensure_ascii=False)

            with open(json_path, "w", encoding="utf-8") as json_file:
                json_file.write(craft_json_end)

            logger.info(f"处理成功: {os.path.abspath(pdf_path)} -> {os.path.abspath(json_path)}")
            return True
        
    except Exception as e:
        # print(f"处理文件 {os.path.basename(pdf_path)} 时发生错误: {e}")
        logger.error(f"处理文件时发生错误 {os.path.abspath(pdf_path)}: {e}")
        return False    

def collect_all_pdfs_for_extract(folder_path, folder_extract_path):
    pdf_paths = []
    processed_files = set()
    for root, dirs, files in os.walk(folder_extract_path):
        for file in files:
    ...
    for root, dirs, files in os.walk(folder_path):
        ...
        for file in files:
            if file.lower().endswith('.pdf'):
                ...
                # 如果未处理过，则添加到待处理列表
                if pdf_filename not in processed_files:
...
if __name__ == '__main__':
    ...
    api_key="7859113da22a475e85c445ed8cd2012d.9cs9uns2CWCNsLsF"

    folder_path = r'C:\Users\howhow\PycharmProjects\AHBF\multi_material_process'  # 待抽取文件夹
    folder_extract_path = folder_path + '_extract' #抽取后文件夹
    os.makedirs(folder_extract_path, exist_ok=True)
    timestamp = time.strftime("%Y%m%d_%H%M%S")
    log_file_path = os.path.join(folder_extract_path, f"processing_{timestamp}.log")
    pdf_paths = collect_all_pdfs_for_extract(folder_path,folder_extract_path)
    ...
    logger = setup_logging(log_file_path)
    logger.info(f"处理完成！总文件数: {total_files}, 成功: {success_count}, 失败: {total_files - success_count}")
