...
parser.add_argument('--center_momentum', default=0, type=float, help='momentum for online center update: default(0.6)')
...
def train(train_loader, model, optimizer, criterion, criterion_T, accuracy, args, rampup_weight):
    ...
    loss_contrastive_avg = utils.RunningAverage()
    ...
    with tqdm(total=len(train_loader)) as t:
        for i, (train_batch, labels_batch) in enumerate(train_loader):
            ...
            # 根据模型类型选择是否传递labels参数（仅对resnet传递）
    ...
    train_metrics = {'train_loss': loss_avg.value(),
                     ...
                     'train_contrastive_loss': loss_contrastive_avg.value(),
    ...
    wandb.log({'trainloss': loss_avg.value(),
               ...
               'trainloss_contrastive':loss_contrastive_avg.value()})
...
def evaluate(test_loader, model, criterion, criterion_T, accuracy, args, rampup_weight):
    ...
    test_metrics = { 'test_loss': loss_avg.value(),
                     ...
                     'test_contrastive_loss': loss_contrastive_avg.value(),
                     'test_accTop1_target': accTop1_avg[0].value(),
                     'time': time.time() - end}
    wandb.log({'testloss': loss_avg.value(),
               'testloss_contrastive': loss_contrastive_avg.value()})  # measure elapsed time
...
def train_and_evaluate(model, train_loader, test_loader, optimizer, criterion, criterion_T, accuracy, model_dir, args):
    ...
    for epoch in range(start_epoch, args.num_epochs):
        ...
        # 在epoch结束时更新历史融合输出和类中心（仅对resnet）
        if "resnet" in args.model and hasattr(model, 'update_epoch_history'):
        ...
        if is_best:
            ...
            utils.save_dict_to_json(test_metrics, os.path.join(model_dir, "test_best_metrics_cifar10_w1d.json"))
