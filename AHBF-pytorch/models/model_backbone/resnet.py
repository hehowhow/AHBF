...
__all__ = ['ResNet', 'resnet32', 'resnet110']
...
def resnet18(pretrained=False, path=None, **kwargs):
    ...
    model = ResNet_i(BasicBlock, [2, 2, 2, 2], **kwargs)
    if pretrained:
...
def resnet34(pretrained=False, path=None, **kwargs):
    ...
    model = ResNet_i(BasicBlock, [3, 4, 6, 3], **kwargs)
    if pretrained:
        model.load_state_dict((torch.load(path))['state_dict'])
    return model
...
def resnet50(pretrained=False, path=None, **kwargs):
    ...
    model = ResNet_i(Bottleneck, [3, 4, 6, 3], **kwargs)
    if pretrained:
        model.load_state_dict((torch.load(path))['state_dict'])
    return model
...
def resnet32(pretrained=False, path=None, **kwargs):
    ...
    model = ResNet(BasicBlock, [5, 4, 5], **kwargs)
    if pretrained:
        model.load_state_dict((torch.load(path))['state_dict'])
    return model
